		...
		do {
			...
			if ( $this->repo ) {
				...
				if ( $this->repo->getReadOnlyReason() !== false ) {
					wfDebug( __METHOD__ . ": repo is read-only");
			...
			if ( !$tmpFile ) {
				wfDebug( __METHOD__ . ": couldn't create temp file" . $thumbPath);
				$thumb = $this->transformErrorOutput( $thumbPath, $thumbUrl, $params, $flags );
			} else {
	...
	public function generateAndSaveThumb( $tmpFile, $transformParams, $flags ) {
		...
		if ( !$thumb ) { // bad params?
			...
			if ( $status->isOK() ) {
				...
				wfDebug( __METHOD__ . ": quickImport failed $tmpThumbPath -> $thumbPath");
